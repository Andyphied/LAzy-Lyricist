...
def get_album_id(ar_id, album_name):
    ...
    for n in albums:
        if n['album']== album_name:
            return get_song_list(n['api_album'])
        

    message = 'Artist not found. You could try out {album_1}, {album_2} or {album_3}'.format(
        album_1 = results[0]['album'], 
        album_2 = results[1]['album'],
        album_3 = results[2]['album']
        )
    return message
...
get_lyrics_text('Portugal. The Man', 'Woodstock')
